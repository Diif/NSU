.LC0:
        .string "stoi"
main:
        pushq   %rbp    # сохраняет указатель на базу стека
        movq    %rsp, %rbp      # сдвигает базу на указатель
        pushq   %rbx
        subq    $168, %rsp
        movl    %edi, -164(%rbp)        # int argc лежит здесь
        movq    %rsi, -176(%rbp)        # **argv здесь
        movl    $0, -24(%rbp)   #  print_tested_value = 0
        cmpl    $1, -164(%rbp)  # сравнение с 1
        jle     .L25 # если argc <= 1, num_of_intervals = 600000000
        leaq    -113(%rbp), %rax # теперь в rax лежит адресс = rbp - 113
        movq    %rax, %rdi  # переложили адрес ячейки стека, чтобы передать в функцию
        call    std::allocator<char>::allocator() [complete object constructor]
        movq    -176(%rbp), %rax # указатель на argv -> rax
        addq    $8, %rax # теперь rax указывает на argv[1]
        movq    (%rax), %rcx    # В rcx теперь строка argv[1][0]
        leaq    -113(%rbp), %rdx # скорее всего что-то из функции вернулось в rbp-113
        leaq    -160(%rbp), %rax # в rax теперь адрес rbp - 160
        movq    %rcx, %rsi  # в rsi теперь строка argv[1][0]
        movq    %rax, %rdi # в rdi теперь адрес rbp - 160
        ...
        movl    %eax, -20(%rbp) # в -20(%rbp) лежит num_of_intervals,num_of_intervals = stoi(argv[1]);
        leaq    -160(%rbp), %rax # ниже, похоже, идут инструкции деструкторов
        movq    %rax, %rdi
        call    std::__cxx11::basic_string<char, std::char_traits<char>, std::allocator<char> >::~basic_string() [complete object destructor]
        leaq    -113(%rbp), %rax
        movq    %rax, %rdi
        call    std::allocator<char>::~allocator() [complete object destructor]
        cmpl    $0, -20(%rbp) # сравнение num_of_intervals == 0)
        jne     .L26  # если !=0, то прыгаем
        movl    $600000000, -20(%rbp) # num_of_intervals = 60000000
        jmp     .L26
.L25:
        movl    $600000000, -20(%rbp)
.L26:
        cmpl    $3, -164(%rbp) 
        jle     .L27    # argc >= 4? тогда print-tested_value = 1.
        movl    $1, -24(%rbp)
.L27: # здесь в блоке опять идет вызов stoi, а затем, в зависимости от значений, вызываются
      # функции. Поведение аналогично предыдущему блоку.
        cmpl    $2, -164(%rbp)
        jle     .L28
        leaq    -65(%rbp), %rax
        movq    %rax, %rdi
        ...
        movq    %rax, %rdi
        call    std::__cxx11::basic_string<char, std::char_traits<char>, std::allocator<char> >::~basic_string() [complete object destructor]
.L29:   # вызов перед последним else
        movl    -20(%rbp), %edx
        movl    -28(%rbp), %eax
        movl    %edx, %esi
        movl    %eax, %edi
        ...
        jmp     .L30
.L28:  # последня else ветка в main функции
        movl    -20(%rbp), %eax
        movl    %eax, %esi
        movl    $1, %edi
        call    MakeNMeasurementsAndPrintToFile(int, int)
.L30:
        movl    $0, %eax
        jmp     .L42    # после прыжка какие-то неисполняемые команды до метки L42
        movq    %rax, %rbx
        leaq    -160(%rbp), %rax
        movq    %rax, %rdi
        call    std::__cxx11::basic_string<char, std::char_traits<char>, std::allocator<char> >::~basic_string() [complete object destructor]
