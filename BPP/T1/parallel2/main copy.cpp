...
void CutAmatrixByCols(Matrix* A_mat, Matrix* local_A_mat);
...
int main(int argc, char** argv) {
  ...
  MPI_Type_vector(N, 1, N, MPI_MATRIX_DATA, &col_type);
  MPI_Type_commit(&col_type);

  Matrix A_mat;
  Matrix vec_b;
  if (!rank) {
    A_mat = CreateZeroMatrix(N, N);
    FormAMatrix(A_mat);
  ...
  int local_rows = sizes_for_scatter[rank];

  Matrix local_vec_x_n = CreateZeroMatrix(local_rows, 1);
  Matrix local_vec_b = CreateUnfilledMatrix(local_rows, 1);
  Matrix local_A_mat = CreateUnfilledMatrix(N, sizes_for_scatter[rank]);

  CutAmatrixByCols(&A_mat, &local_A_mat);
  ...
  /*  PrepareGlobalVars();
    ...
    MPI_Scatterv(A_mat.matrix, sizes_for_scatter, displs, MPI_DOUBLE,
                 local_A_mat.matrix, local_rows, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    ...
    while (streak != 5) {
      ...
      UpdateR(r_prev, local_A_mat, z_n, alpha_n);
...
MATRIX_DATA GetNextAlpha(Matrix& r_n, Matrix& local_A_mat, Matrix& z_n) {
  ...
  return ScalarMult(r_n, r_n) / ScalarMult(vector_buffer, z_n);
}
...
MATRIX_DATA GetNorm(Matrix& vector) {
  ...
  return sqrt(ScalarMult(vector, vector));
}
...
void SubMatrixFromMatrix(Matrix& matrix_for_result, Matrix& matrix_from,
  ...
  if (matrix_from.rows != matrix_to.rows ||
      matrix_from.columns != matrix_to.columns) {
    throw(std::invalid_argument("Can't sub matrices."));
  }
...
MATRIX_DATA ScalarMult(Matrix& vec1, Matrix& vec2) {
  ...
  MPI_Reduce(&sum, &result, 1, MPI_MATRIX_DATA, MPI_SUM, 0, MPI_COMM_WORLD);
  MPI_Bcast(&result, 1, MPI_MATRIX_DATA, 0, MPI_COMM_WORLD);
  return result;
}
...
void CalculateSizesAndDisplsForScatter(int num_of_proc, int max_rows,
                                       int max_columns, int** sizes,
                                       int** displs) {
  int base = max_columns / num_of_proc;
  int remainder = max_columns % num_of_proc;
...
void CutAmatrixByCols(Matrix* A_mat, Matrix* local_A_mat) {
  for (int i = 0; i < num_of_proc; i++) {
    int cols = sizes_for_scatter[i];
    int offset = displs[i];
    MPI_Datatype local_col;
    MPI_Type_vector(N, 1, local_A_mat->columns, MPI_MATRIX_DATA, &local_col);
    MPI_Type_commit(&local_col);
    for (int j = 0; j < cols; j++) {
      if (!rank) {
        fprintf(stderr, "SEND TO %d --------\n", i);
        MPI_Send(((*A_mat).matrix) + j + offset, 1, col_type, i, 0,
                 MPI_COMM_WORLD);
      }
      if (rank == i) {
        MPI_Recv(((*local_A_mat).matrix) + j, 1, local_col, 0, 0,
...
void MultColumnsOnVec(Matrix* mat, Matrix* vec) {
  if (mat->columns != vec->rows || vec->columns != 1) {
    throw(std::invalid_argument("Wrong cls and rows in parralel case"));
  }
  int vec_size = vec->rows;
  int size = mat->columns * mat->rows;
  for (int i = 0; i < size; i++) {
  }
}