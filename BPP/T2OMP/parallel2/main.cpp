...
#pragma omp barrier
    Matrix local_mat = Get_r0(vec_b, vec_x_n, A_mat);
#pragma omp single nowait
    {
      r_n.rows = local_mat.rows;
      r_n.columns = local_mat.columns;
      r_n.matrix = local_mat.matrix;
      shared_matrix.matrix = NULL;
      shared_matrix.rows = 0;
      shared_matrix.columns = 0;
      z_n = Get_z0(r_n);
    }
...
#pragma omp single
  {
    shared_matrix = CreateUnfilledMatrix(A_mat.rows, vec_x0.columns);
    PrepareMatricesForMult(buffer, A_mat, vec_x0);
  }
  MultMatrixOnMatrix(buffer, A_mat, vec_x0);
  SubMatrixFromMatrix(shared_matrix, vec_b, buffer);
